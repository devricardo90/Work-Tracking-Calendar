...
function buildPages(userName: string, month: string, entries: ReportEntry[]) {
  ...
  const createBasePage = (pageNumber: number): PdfPage => {
    const commands = [
      ...
      setFont("mono", 10),
      textAt(48, 630, "DATE        STATUS      LOCATION                    HOURS  NOTES"),
      textAt(48, 618, "----------------------------------------------------------------------------"),
      setFont("sans", 9),
    ...
    for (const entry of entries) {
      const status =
        entry.entryStatus === "worked"
          ? "WORKED"
          : entry.entryStatus === "day-off"
            ? "DAY OFF"
            : "NO WORK";
      const location = entry.location
        ? entry.location.length > 26
          ? `${entry.location.slice(0, 23)}...`
          : entry.location
        : "-";
      const notes = entry.notes
        ...
        : "-";
      pushRow(
        `${formatDayLabel(entry.workDate)}  ${status.padEnd(10, " ")}  ${location.padEnd(26, " ")}  ${Number(entry.hoursWorked)
          .toFixed(1)
          .padStart(5, " ")}  ${notes}`,
      );
    }
